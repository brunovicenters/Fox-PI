...
<x-layout>
    <div class="flex gap-9 ">
        <h1 class="hanalei text-6xl drop-shadow-md mb-5 text-azul">Minha Conta</h1>
        <div>
    ...
    <form class="flex items-right flex-col w-2/5 text-laranja-escuro drop-shadow-md space-y-1" action="{{ route('update.minha-conta') }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="w-full justify-end space-x-3 flex drop-shadow-md">
            <a onclick="edit()" class="hidden fields botao poppins text-white rounded-lg px-3 py-2 uppercase font-bold drop-shadow-md hover:scale-105 hover:drop-shadow-lg ease-linear h-11 mt-2">Cancelar</a>
            <p class="values"><p>
